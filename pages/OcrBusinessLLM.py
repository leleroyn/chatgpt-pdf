...
def main():
    ...
    st.subheader(f"🏢 企业营业执照信息提取(OCR+{llm})")
    
    # Create a more organized layout
    head_col1, head_col2 = st.columns([3, 1])
    with head_col1:
        uploaded_file = st.file_uploader("上传营业执照影像", type=["png", "jpg", "bmp", "jpeg"])
    with head_col2:
        ...
        try:
            ...
            with col2:
                ...
                with st.spinner("正在检测营业执照信息..."):
                    ...
                    start = time()
                    results = ips_service.bizlic_preprocess(byte_data, return_corp_image=True, return_ocr_text=True,
                                                         tool=(conf_size, True, False))
                    
                    if not results:
                        st.info("没有检测到任何营业执照。请尝试调整置信度阈值或上传更清晰的图像。")
                        return
                    ...
                    for i, res in enumerate(results):
                        ...
                        bizlic_type = ips_service.convert_bizlic_type(res['bizlic_type'])
                        
                        # Only show results above confidence threshold
                        if confidence >= conf_size:
                            st.success(f"检测到营业执照 #{valid_results_count+1}")
                            st.markdown(f"**类型**: {bizlic_type}")
                            st.markdown(f"**置信值**: {confidence:.2f}")
                            ocr_result.append(res["ocr_text"])
                            valid_results_count += 1
                            st.divider()
                        else:
                            st.warning(f"检测到营业执照 #{i+1} (置信值: {confidence:.2f}) - 低于阈值被过滤")
                    ...
                    st.info(f"共检测到 {valid_results_count} 个有效营业执照")
            ...
            with st.spinner("正在提取营业执照信息..."):
                ...
                llm_result = oneApiService.ocr_business_llm(ocr_text)
                end = time()
                elapsed3 = end - start
            ...
            with detail_col1:
                ...
                st.markdown("#### 📋 检测详情")
                tabs = st.tabs([f"营业执照 #{i+1}" for i in range(len([r for r in results if r['confidence'] >= conf_size]))])
                ...
                for i, res in enumerate(results):
                    ...
                    if confidence >= conf_size:
                        with tabs[valid_result_index]:
                            st.markdown(f"**类型**: {ips_service.convert_bizlic_type(res['bizlic_type'])}")
                            st.markdown(f"**置信值**: {confidence:.2f}")
                            
                            # Display cropped image (hidden by default)
                            try:
                                if res.get("corp_image_base64"):
                                    cropped_image = ips_service.base64_to_pil(res['corp_image_base64'])
                                    with st.expander("查看裁剪图像"):
                                        st.image(cropped_image, caption=f"裁剪图像 #{valid_result_index+1}", use_column_width=True)
